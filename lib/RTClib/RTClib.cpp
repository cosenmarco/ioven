...
DateTime RTC_DS1307::now() {
  ...
  uint8_t ss = Wire._I2C_READ();
  uint8_t mm = Wire._I2C_READ();
  uint8_t hh = Wire._I2C_READ();
  Wire._I2C_READ();
  uint8_t d = Wire._I2C_READ();
  uint8_t m = Wire._I2C_READ();
  uint8_t y = Wire._I2C_READ();

  return DateTime (bcd2bin(y) + 2000,
    bcd2bin(m), bcd2bin(d), bcd2bin(hh), bcd2bin(mm),
    bcd2bin(ss & 0x7F));
}
